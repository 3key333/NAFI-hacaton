...
Лендинг платформы НАФИ для оценки цифровых компетенций с конфигуратором подключения и 5-шаговым визардом оформления.
...
2. Выбирает тип плательщика и заполняет контактные данные (шаг 2) — валидация на переход «Далее».
3. Принимает mock-договор (шаг 3).
4. Выбирает способ оплаты и нажимает «Оплатить» (шаг 4).
5. Видит экран успеха (шаг 5); кнопка сброса вызывает `resetWizard()`. Повторный `openWizard()` после успеха тоже сбрасывает визард на шаг 1.
...
|-----|-----------|------------|
| 1 | `WizardStep1` | Параметры + стоимость (`ConfiguratorPanel` + детализация) |
| 2 | `WizardStep2` | Плательщик + контактные данные |
...
Названия шагов в боковой навигации — `WIZARD_STEPS` в `landingData.ts`.

### Валидация при переходе между шагами

- **Шаг 2 → 3:** `validateConnectionForm()` — имя, фамилия, email, согласие на ПДн.
...
Хук **`useConnection()`** — единая точка доступа для компонентов. Метод `openWizard()` прокручивает страницу к `#wizard`; если визард на шаге «Готово», сначала вызывает `resetWizard()`.
